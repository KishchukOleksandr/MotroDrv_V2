...
__weak void MCP_ReceivedFrame(MCP_Handle_t *pHandle, uint8_t Code, uint8_t *buffer, uint8_t Size)
{
  ...
  uint8_t bErrorCode;
  static FloatToU32 Position;
  static FloatToU32 Duration;
  ...
  case MC_PROTOCOL_CODE_SET_POSITION_CMD:
    {
      ...
      Duration.Float_Val = (Position.Float_Val)/90;
      if(Duration.Float_Val < 0)
      {
        Duration.Float_Val = -Duration.Float_Val;
      }
      Duration.Float_Val = Duration.Float_Val < 0.5f ? 0.5f : Duration.Float_Val;

      Position.Float_Val = (Position.Float_Val * 2 * M_PI)/360;      
