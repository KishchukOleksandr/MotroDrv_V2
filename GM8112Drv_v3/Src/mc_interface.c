...
__weak void MCI_ExecPositionCommand( MCI_Handle_t * pHandle, float FinalPosition, float Duration )
{
  ...
  static float currentPositionRad;
  
  currentPositionRad = (float)(SPD_GetMecAngle(STC_GetSpeedSensor(pHandle->pSTC))) / RADTOS16;
  
  if (Duration > 0) 
  {    
    TC_MoveCommand(pHandle->pPosCtrl, currentPositionRad, FinalPosition, Duration);
  }
